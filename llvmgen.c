/* llvmgen.c — Z Compiler LLVM IR backend
   ...
   Pipeline: Z source → this file → /tmp/_zc_PID.ll → clang -O3 → binary
*/
#include "llvmgen.h"
#include <stdlib.h>
#include <string.h>
...
int llvmgen(ASTNode *prog, Scope *global_scope,
    ...
    char tmp_path[256];
    snprintf(tmp_path, sizeof(tmp_path), "/tmp/_zc_ll_body_%d.ll", (int)getpid());
    ...
    fprintf(out, "target triple = \"arm64-apple-macosx26.0.0\"\n\n");
